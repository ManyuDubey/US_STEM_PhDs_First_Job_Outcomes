...
<body>
  ...
  <script>
    ...
    function renderChart(container, title, subtitle, series, opts = {{}}) {{
      ...
      const seriesLookup = filteredSeries.map((s) => {{
        const points = s.values.map((v) => {{
          const plotValue = mode === 'count' ? (v.value || 0) : v.share;
          return {{
            series: s.name,
            color: s.color,
            label: s.label,
        ...
        for (let i = 1; i < points.length; i++) {{
          segments.push({{...points[i], x1: points[i - 1].x, y1: points[i - 1].y, x2: points[i].x, y2: points[i].y}});
        }}
      ...
      function renderSelection(yearEntry) {{
        clearOverlay();
        if (!yearEntry && !selectedSeries) {{
          tooltip.style.opacity = '0';
          selectedYear = null;
          return;
        }}
        selectedYear = yearEntry ? yearEntry.year : null;
        if (selectedSeries) {{
          const selected = seriesLookup.find((s) => s.name === selectedSeries);
          if (selected) {{
            overlayCtx.fillStyle = 'rgba(255, 253, 248, 0.68)';
            overlayCtx.fillRect(margin.left, margin.top, plotW, plotH);
            overlayCtx.strokeStyle = selected.color;
            overlayCtx.lineWidth = 4;
            overlayCtx.beginPath();
            selected.points.forEach((point, i) => {{
              if (i) overlayCtx.lineTo(point.x, point.y); else overlayCtx.moveTo(point.x, point.y);
            }});
            overlayCtx.stroke();
            overlayCtx.fillStyle = selected.color;
            selected.points.forEach((point) => {{
              overlayCtx.beginPath();
              overlayCtx.arc(point.x, point.y, 4.8, 0, Math.PI * 2);
              overlayCtx.fill();
        ...
        if (!yearEntry) {{
          const selected = seriesLookup.find((s) => s.name === selectedSeries);
          tooltip.innerHTML = selected ? `<strong>${{selected.label}}</strong><br>Click a point for year details<br>Click empty chart space to clear` : '';
          tooltip.style.opacity = selected ? '1' : '0';
          tooltip.style.left = (margin.left + 16) + 'px';
      ...
      function nearestPoint(clientX, clientY) {{
        ...
        yearLookup.forEach((yearEntry) => {{
          yearEntry.points.forEach((point) => {{
            ...
            if (dist < bestDist) {{
              bestDist = dist;
              best = {{point, yearEntry}};
            }}
          }});
        }});
        return {{...best, distance: bestDist, x: mx, y: my}};
      }}
      ...
      function nearestSeriesHit(clientX, clientY) {{
        const pointHit = nearestPoint(clientX, clientY);
        let bestSeries = null;
        let bestDist = Infinity;
        seriesLookup.forEach((s) => {{
          s.segments.forEach((seg) => {{
            const dist = distanceToSegment(pointHit.x, pointHit.y, seg);
            if (dist < bestDist) {{
              bestDist = dist;
              bestSeries = s.name;
            }}
      ...
      canvas.addEventListener('mouseleave', () => {{
        if (selectedYear === null && selectedSeries === null) {{
          tooltip.style.opacity = '0';
        }}
      }});
