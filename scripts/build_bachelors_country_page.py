...
def write_page(payload: dict[str, object]) -> None:
    ...
    <div class="card">
      ...
      <div id="point-details" class="note">Click a line to isolate that country. Click a point to pin its year, count, and share.</div>
  ...
  <script>
    ...
    function fillUniversities() {{
      universitySelect.innerHTML = universities.map((u, i) => `<option value="${{i}}">${{u[0]}}</option>`).join('');
    }}
    function fillFields() {{
      const groups = groupsForSelectedView();
    ...
    function drawChart(group, yearStart, yearEnd) {{
      ...
      if (!series.length) {{
        ctx.fillStyle = '#666';
        ctx.font = '15px Georgia';
        ctx.fillText('No bachelor-country data for this year range.', 20, 40);
        return;
      }}
    ...
    function renderPointDetails(point) {{
      if (!point) {{
        pointDetails.innerHTML = 'Click a line to isolate that country. Click a point to pin its year, count, and share.';
        return;
      }}
      pointDetails.innerHTML = `<strong>${{point.series}} · ${{point.year}}</strong>: ${{pct(point.share)}} · ${{fmt(point.count)}} / ${{fmt(point.total)}} PhDs with bachelor country`;
    }}
    ...
    document.getElementById('country-chart').addEventListener('mousemove', (event) => {{
      ...
      const bestDist = hit.distance;
      if (!best || bestDist > 12) {{
        tooltip.style.opacity = 0;
        return;
      }}
      tooltip.innerHTML = `<strong>${{best.series}}</strong><br>${{best.year}}: ${{pct(best.share)}}<br>${{fmt(best.count)}} / ${{fmt(best.total)}}<br>Click to isolate`;
    ...
    universitySelect.addEventListener('change', () => {{ selectedSeries = null; fillFields(); }});
    viewSelect.addEventListener('change', () => {{ selectedSeries = null; fillFields(); }});
    fieldSelect.addEventListener('change', () => {{ selectedSeries = null; updateYearBounds(); }});
